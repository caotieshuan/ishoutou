...
class MsgonlineAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$uidid2=$msgconfig['sms']['userid2']; //分配给你的账号
		...
		$uid5=$msgconfig['sms']['user5']; //骏逸通达账号
		...
		if($type==0){
			...
			$gateway = "http://125.208.3.91:8888/sms.aspx?action=overage&userid={$uidid2}&account={$uid2}&password={$pwd2}";
			$result = file_get_contents($gateway);
			$xml = simplexml_load_string($result);

			if(strcmp(trim($xml->returnstatus),'Sucess')==0)
			{
				$d = $xml->overage;
			}else{
				$d = $xml->message;
			...
			$gateway = "http://dx.ipyy.net/sms.aspx?action=overage&userid=&account={$uid4}&password={$pwd4}";
			$result = file_get_contents($gateway);
			$xml = simplexml_load_string($result);
			...
			{
				//echo $xml->overage;
				//echo $xml->sendTotal;exit;
				$d = $xml->overage;
				$this->assign('yy',$d);
			}
		}else if($type==5){
			$password = base64_encode($pwd5);
			$gateway = "http://218.241.153.225:6100/servlet/UserServiceAPI?method=getRestMoney&username={$uid5}&password={$password}";
			$result = file_get_contents($gateway);
            $xml = strpos($result,'memberSmsNum') !==false?true:false;
			if($xml)
			{
				$templet = explode(';',$result);
				$templet = explode('=',$templet[0]);
				$this->assign('jytd',$templet[1]);
			}
		}else{
		...
		$this->assign("type_list", array("3"=>'关闭短信平台服务',"1"=>'短信通道2',"4"=>'阳洋短信提供商',"5"=>'骏逸通达提供商'));
        $this->display();
    }
