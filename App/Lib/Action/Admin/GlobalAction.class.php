...
class GlobalAction extends ACommonAction
{
    ...
    */
    ...
    {
		$this->assign('media_position', C('FRIEND_LINK'));
		...
		$count  = $Media->where($condition)->count(); // 查询满足要求的总记录数   
		...
		$list = $Media->field($fields)->where($condition)->order($order)->limit($Page->firstRow.','.$Page->listRows)->select();

		$MediaList = $list;
		$Media_p = C('FRIEND_LINK');
			  
		foreach($MediaList as $key => $v){
			foreach($v as $key_s => $v_s){
				if($key_s == 'link_type') $v_s = $Media_p[$v_s];
				elseif($key_s == 'game_name' && empty($v_s)) $v_s = "无";
				else if($key_s == 'is_show'){
					if($v_s==1) $v_s="是";
					else $v_s="否";
				}
				$MediaList[$key][$key_s] = $v_s;
			}
		} 
		
		$MediaArr['MediaList'] = $MediaList;
		$MediaArr['PageBar'] = $show;



		$this->assign('media_list', $MediaArr['MediaList']);
		$this->assign('pagebar', $MediaArr['PageBar']);
		$this->assign('position', "媒体报道");
        $this->display();
    }
    ...
    public function addMedia()
    {
		...
		if(!isset($_POST['fid'])){//新增
			...
			$this->assign('jumpUrl', U('/admin/global/media/'));
			$this->success('媒体报道添加成功');
		}else{//编辑
		
			$data['id']=intval($_POST['fid']);
			$newid = M('media')->save($data);
			if(!$newid>0){
    ...
    public function doDeleteMedia()
    {
		...
		$delnum = M('media')->where("id in ({$idarray})")->delete(); 
		
		if($delnum){
			$a_data['success'] = $rid;
			$a_data['success_msg'] = "媒体报道删除成功";
			$a_data['aid'] = $idarray;
		}else{
			$a_data['success'] = 0;
			$a_data['error_msg'] = "媒体报道删除失败";
		}
...
/*     public function cleanall()
    {
		...
		echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'>\"".$dirs[0]."\" 目录下缓存清除成功! </div> <br /><br />";
		
	} */
	
	/*清Runtime目录下文件目录 保留自身不影响权限 @author:Bob @time:2015/4/28 */
	function cleanall() {
		$dir = C('APP_ROOT').'Runtime';
		$od=opendir($dir);
		...
		closedir($od);
		alogs("Global",0,1,'执行了所有缓存清除操作！');//管理员操作日志
		echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'>\"".$dir."\" 目录下缓存清除成功! </div> <br /><br />";
	}
    ...
    //i主播
    public function izhubo()
    {
		$this->assign('media_position', C('IZHUBO_LINK'));
		import("ORG.Util.Page");
		...
		$list = $Media->field($fields)->where($condition)->order($order)->limit($Page->firstRow.','.$Page->listRows)->select();
		$MediaList = $list;
		$zhubo_p = C('IZHUBO_LINK');
			  
		foreach($MediaList as $key => $v){
			foreach($v as $key_s => $v_s){
				if($key_s == 'link_type') $v_s = $zhubo_p[$v_s];
				else if($key_s == 'is_show'){
					if($v_s==1) $v_s="是";
					else $v_s="否";
				}
		...
		$this->assign('media_list', $MediaArr['MediaList']);
		$this->assign('pagebar', $MediaArr['PageBar']);
		$this->assign('position', "i主播");
        $this->display();
    }
    public function searchIzhubo()
    {
		$this->assign('media_position', C('IZHUBO_LINK'));
		//搜索
		import("ORG.Util.Page");
		if($_POST){
			...
			$searchKey = array('link_txt','link_type','is_show');
			foreach($data as $key => $v){
				if(in_array($key,$searchKey)){
					if(!empty($v)) $condition[$key]=array('eq',EnHtml($v));
				}
			
			}
		}
		if($_REQUEST['link_txt']){
			$search['link_txt'] = urldecode($_REQUEST['link_txt']);
		}
		if($_REQUEST['link_type']){
			$search['link_type'] = urldecode($_REQUEST['link_type']);
		}
		if($_REQUEST['is_show']){
			$search['is_show'] = urldecode($_REQUEST['is_show']);
		}
		$Media = M('izhubo');
		$page_size = ($page_szie==0)?C('ADMIN_PAGE_SIZE'):$page_szie;
	...
	public function addIzhubo()
    {
		...
		if(!isset($_POST['fid'])){//新增
			...
			$this->assign('jumpUrl', U('/admin/global/izhubo/'));
			$this->success('添加成功');
		}else{//编辑
		
			$data['id']=intval($_POST['fid']);
			$newid = M('izhubo')->save($data);
			if(!$newid>0){
				$this->error('编辑失败，请确认填入数据正确');
				exit;
			}
