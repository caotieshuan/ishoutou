...
function get_personal_benefit($uid)
{
    ...
    $investor = M("investor_detail")
        ...
        ->where(' status =2 and  investor_uid=' . $uid)->find();
    $investor['interest'] += $investor['fee'];//共赚利要不在减去管理费
   ...
   $log = get_money_log($uid);
    ...
    $benefit['interest_collection'] = $interest_collection['interest'] + $transfer_interest_collection['interest'];//dai shou ben xi
    ...
    //红包奖励
    $redbag = M('member_moneylog')->where("uid={$uid}  and type=49")->find();//红包
    $benefit['redmoney'] =  $redbag['affect_money'];
   //其他奖励
    $redbag = M('member_moneylog')->field('sum(affect_money) affect_money')->where("uid={$uid}  and type=55")->find();//其他 奖励
    $benefit['othermoney'] =  $redbag['affect_money'];
    $benefit['total'] = $benefit['ireward'] + $benefit['spread_reward'] + $benefit['con_reward'] + $benefit['re_reward'] + $benefit['interest']+$benefit['othermoney']+$benefit['redmoney'];
...
function hidecard($cardnum, $type = 1, $default = "")
{
    ...
    if ($type == 1) $cardnum = substr($cardnum, 0, 10) . str_repeat("*", 8) . substr($cardnum, strlen($cardnum) - 0);//身份证
    elseif ($type == 2) $cardnum = substr($cardnum, 0, 2) . str_repeat("*", 5) . substr($cardnum, strlen($cardnum) - 2);//手机号
    ...
    elseif ($type == 4)  $cardnum = subString($cardnum,0,1);//新用户名
    elseif ($type == 5) $cardnum = mb_substr($cardnum, 0, 1,'utf-8') . str_repeat("*", 3) . mb_substr($cardnum, strlen($cardnum) - 1,2,'utf-8');//新用户名
    //elseif ($type == 5) $cardnum = subString($cardnum,0,1);//新用户名
    return $cardnum;
}
...
function getLeveIco($num, $type = 1)
{
    ...
    foreach ($leveconfig as $key => $v) {
        if ($num >= $v['start'] && $num <= $v['end']) {
            ...
            elseif ($type == 2) return '<a  target="_blank" href="javascript:/*DQ*/;"><img src="' . __ROOT__ . '/UF/leveico/' . $v['icoName'] . '" title="' . $v['name'] . '"/></a>';
...
{
    if (!is_numeric($money) || empty($money)) return "￥0.00";
    $sb = "";
    if ($money < 0) {
        $sb = "-";
        $money = $money * (-1);
...
function sendsms($mob,$content){
    ...
    //同一个IP一个小时内只能发送20条验证信息
    //if(20 < M('sendlog')->where(array('clientip'=>get_client_ip(),'dateline'=>array('egt',time()-3600)))->count()){
     //   $data['result'] = '发生错误，ip一小时内超过20条信息';
     //   $data['status'] = 1;
        //同一个手机号内10分钟内只能发送5条短信
    if( 15 < M('sendlog')->where(array('phone'=>$mob,'dateline'=>array('egt',time()-86400)))->count()){
        $data['result']  = '发生错误，手机今天内发送短信超过15条。';
        $data['status'] = 2;
        //同一个手机号一分钟内只发送一个验证码
    }else{
        ...
        if(1 == $type){
            $data['smstype']=1;
            $userid=$msgconfig['sms']['userid2']; //分配给你的账号
            $account=$msgconfig['sms']['user2']; //分配给你的账号
            $password=$msgconfig['sms']['pass2']; //密码
            $gateway = "http://125.208.3.91:8888/sms.aspx?action=send&userid={$userid}&account={$account}&password={$password}&mobile={$mob}&content={$content}&sendTime=";
            $result = file_get_contents($gateway);
            $xml = simplexml_load_string($result);
            $result = 0 == strcmp(trim($xml->returnstatus), 'Success');
        }else  if(5 == $type){
            $data['smstype']=1;
            $userid=$msgconfig['sms']['user5']; //账号
            $password=base64_encode($msgconfig['sms']['pass5']); //密码
            $content = urlencode(mb_convert_encoding($content, 'GBK', 'UTF-8'));
            $gateway = "http://218.241.153.225:6100/servlet/UserServiceAPI?method=sendSMS&username={$userid}&password={$password}&mobile={$mob}&content={$content}&isLoginSms=0&extno=2521&smstype=2";
            $result = file_get_contents($gateway);
            $xml=iconv("GBK", "UTF-8", $result);
            $result = strpos($result,'success') !==false?true:false;
        }else{
            $data['smstype'] = 2;
            $userid = '';//企业ID $userid
            $account = $msgconfig['sms']['user4']; //用户账号 $account
            //$password = 'xd00030678';
            $password = $msgconfig['sms']['pass4']; //用户密码 $password
            $gateway = "http://114.113.154.5/sms.aspx?action=send&userid={$userid}&account={$account}&password={$password}&mobile={$mob}&content={$content}&sendTime=";
            $result = file_get_contents($gateway);
            $xml = simplexml_load_string($result);
            $result = 0 == strcmp(trim($xml->returnstatus), 'Success');
        }
...
#isU', $v, $ar[$k]);
        ...
        return 0 == strcmp(trim($xml->returnstatus), 'Success');
        ////////////////////////////////////////////////////////阳洋短信接口 结束/////////////////////////////////////////////
    } else {
        return false;
    }
...
function getVerify($uid)
{
    ...
    if ($vo['user_phone']) $str .= '&nbsp;<img alt="手机认证通过" src="' . __ROOT__ . '/Style/H/images/icon/phone.gif"/>';
...
{
    $pre = C('DB_PREFIX');
    $vo = M("members m")->field("m.id,m.user_phone,m.user_leve,m.time_limit,m.pin_pass,s.id_status,s.phone_status,s.email_status,s.video_status,s.face_status")->join("{$pre}members_status s ON s.uid=m.id")->where("m.id={$uid}")->find();
    $str = "";
    ...
    if ($vo['user_phone']) $str .= '<a href="' . __APP__ . '/member/verify#fragment-2"><img alt="手机认证通过"   title="手机认证通过" src="' . __ROOT__ . '/Style/H/images/icon/phone.gif"/>&nbsp;';
...
function getVerifymobile_ucenter($uid)
{
    $pre = C('DB_PREFIX');
    $vo = M("members m")->field("m.id,m.user_phone,m.user_leve,m.time_limit,m.pin_pass,s.id_status,s.phone_status,s.email_status,s.video_status,s.face_status")->join("{$pre}members_status s ON s.uid=m.id")->where("m.id={$uid}")->find();
    $str = "";
    $str .= '<span onclick="window.location.href=\'/member/verify/idcards?id=1#fragment-3\'" class="ico1'.($vo['id_status'] == 1 ? '' : ' not').'"></span>';
    $str .= '<span onclick="window.location.href=\'/member/verify?id=1#fragment-2\'" class="ico2'.($vo['user_phone'] ? '' : ' not').'"></span>';
    $str .= '<span onclick="window.location.href=\'/member/user/pin#fragment-3\'" class="ico3'.($vo['pin_pass'] ? '' : ' not').'"></span>';
    return $str;
}
...
{
    $binfo = M("borrow_info")->field('borrow_uid,borrow_money,borrow_type,repayment_type,borrow_interest_rate,borrow_duration,has_vouch,has_borrow,borrow_max,borrow_min,can_auto')->where('borrow_type <> 3')->find($borrow_id);
    if ($binfo['can_auto'] == '0') {
        return;
    }
    $map['a.is_use'] = 1;
    ...
    foreach ($autolist as $key => $v) {
        ...
        $invest_money = intval($v['invest_money']);
        $borrow_max = intval($binfo['borrow_max']);

        if ($invest_money > $borrow_max && $borrow_max > 0) { // 大于最大投标 且设置最大投标
            $investMoney = floatval($binfo['borrow_max']);
        }
