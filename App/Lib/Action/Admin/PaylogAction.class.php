...
class PaylogAction extends ACommonAction
{
    ...
    */
	...
	public function doEdit(){
		...
		if($status==1){
			...
			if($vo['way']=="off")  SMStip("nopayoffline",$vx['user_phone'],array("#USERANEM#","#MONEY#"),array($vx['user_name'],$vo['money']));
			$save['deal_user'] = session('adminname');
			$save['deal_uid'] = $this->admin_id;
			$save['status'] = 3;
	...
	public function _listFilter($list){
	 	$listType = C('PAYLOG_TYPE');
	 	$payType = array('off'=>'线下充值','gfb'=>'国付宝','ips'=>'环迅支付','chinabank'=>'京东支付','baofoo'=>'宝付','shengpay'=>'盛付通','tenpay'=>'财付通','llpay'=>'连连支付','ecpss'=>'汇潮支付','easypay'=>'易生支付','cmpay'=>'中国移动支付');
    ...
    */
    public function paylogonline()
    {
		...
		$this->assign('type_list',$listType);
		$field= 'id,uid,status,money,add_time,tran_id,way,off_bank,off_way,deal_user';
		$map['way']=array("in",'gfb,ips,chinabank,baofoo,shengpay,tenpay,ecpss,llpay,easypay,cmpay');
		$this->_list(D('Paylog'),$field,$map,'id','DESC',$xtime);
        $this->display();
    }
...
//					$ret = sendsms($vx['user_phone'],$info);
	...
	public function paylogalipay() {
		...
		$count = M("member_alipay")->where($map)->count();
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$status = array(1=>"充值未处理",2=>"充值成功",3=>"充值失败");
		$Lsql = "{$p->firstRow},{$p->listRows}";

		$list = M("member_alipay")->limit($Lsql)->order("id DESC")->where($map)->select();
		//print_R($list);die;
