 ...
 */
    ...
    public static function getHttpResponsePOST($url, $cacert_url, $para, $input_charset = '')
    {
        try{
           ...
           // echo ( curl_error($curl) );//如果执行curl过程中出现异常，可打开此开关，以便查看异常内容
           // exit;
            curl_close($curl);
        }catch (Exception $e){
    ...
    public function post_url($url, $data, $type='json', $time = 10){
        try{
            $ch = curl_init();
            if(false === $ch)
                throw new Exception('failed to initialize');
    ...
    public static function charsetEncode($input, $_output_charset, $_input_charset)
    {
        ...
        } else throw new Exception("sorry, you have no libs support for charset change.");
        return $output;
    }
    ...
    public static function charsetDecode($input, $_input_charset, $_output_charset)
    {
        ...
        if ($_input_charset == $_output_charset || $input == null) {
            ...
            $output = iconv($_input_charset, $_output_charset, $input);
        } else throw new Exception("sorry, you have no libs support for charset changes.");
        return $output;
    }
    ...
    public static function getJsonVal($json, $k)
    {
        if (isset($json->{$k})) {
            return trim($json->{$k});
        }
