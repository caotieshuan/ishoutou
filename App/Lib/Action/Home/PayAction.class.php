...
class PayAction extends HCommonAction {
	...
	public function chinabank(){
		...
		///判断订单号是否存在
		$submitdata['v_oid'] = date("Ymd").'-'.$submitdata['v_mid'].'-'.mt_rand( 100000,999999);
		while(M('member_payonline')->where(array('pay_no'=>$submitdata['BillNo']))->count()){
			$submitdata['v_oid'] = date("Ymd").'-'.$submitdata['v_mid'].'-'.mt_rand( 100000,999999);
		}
...
*/		$submitdata['v_ordername'] =$vo['user_name'];
		...
		M('member_payonline')->add($this->paydetail);
		$this->create($submitdata,"https://tmapi.jdpay.com/PayGate");
	}
	...
	public function ecpss(){
		...
		$submitdata['Remark'] = "";
		$submitdata['defaultBankNumber'] = $this->paydetail['bank'];
		$submitdata['orderTime'] = date("YmdHis");
		////////////////////////////////////////
		...
		//unset($this->paydetail['bank']); 保存下充值银行
		...
		$this->paydetail['pay_no'] = $submitdata['BillNo'];
		$result = M("member_payonline" )->add( $this->paydetail);
		if(!$result) exit('error:订单生成失败，请刷新页面重试');
	...
	public function easypay(){
		...
		$submitdata['return_url'] = "http://10.68.76.106:8088/test3/return_url.php";
		$submitdata['notify_url'] = "http://10.68.76.106:8088/test3/notify_url.php";
		...
		$submitdata['paymethod'] = "bankDirect";//支付方式
		$submitdata['defaultbank'] = $this->paydetail['bank'];
