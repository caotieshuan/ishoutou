...
class CapitalaccountAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		//余额
		if(!empty($_REQUEST['bj1'])  && !empty($_REQUEST['money'])){
				if($_REQUEST['bj1']=='gt'){
					$bj = '>';
				}else if($_REQUEST['bj1']=='lt'){
					$bj = '<';
				}else if($_REQUEST['bj1']=='eq'){
					$bj = '=';
				}
				$map['_string'] = "(mm.account_money+mm.back_money) ".$bj.$_REQUEST['money'];
			
			$search['bj1'] = $_REQUEST['bj1'];	
			$search['money'] = $_REQUEST['money'];	
		}
		//冻结金额 
		if(!empty($_REQUEST['bj2'])  && !empty($_REQUEST['money_freeze'])){
			
				$map['mm.money_freeze'] = array($_REQUEST['bj2'],$_REQUEST['money_freeze']);
				$search['bj2'] = $_REQUEST['bj2'];	
				$search['money_freeze'] = $_REQUEST['money_freeze'];	
		}
		//待收金额
		if(!empty($_REQUEST['bj3'])  && !empty($_REQUEST['money_collect'])){
			
				$map['mm.money_collect'] = array($_REQUEST['bj3'],$_REQUEST['money_collect']);
				$search['bj3'] = $_REQUEST['bj3'];	
				$search['money_collect'] = $_REQUEST['money_collect'];	
		}
		//分页处理
	...
	public function yott()
	{
		...
		if(!empty($_REQUEST['res'])){
			$map['l.status'] = 1 == $_REQUEST['res'] ? 1 : 0;
			$search['status'] = $_REQUEST['status'];
		}
		if(!empty($_REQUEST['status'])){
			$map['my.status'] = $_REQUEST['status'];
			$search['status'] = $_REQUEST['status'];
		}

		if(!empty($_REQUEST['start_time']) && !empty($_REQUEST['end_time'])){
			$timespan = strtotime(urldecode($_REQUEST['start_time'])).",".strtotime(urldecode($_REQUEST['end_time']));
			$map['l.dateline'] = array("between",$timespan);
			$search['start_time'] = urldecode($_REQUEST['start_time']);
			$search['end_time'] = urldecode($_REQUEST['end_time']);
		}elseif(!empty($_REQUEST['start_time'])){
			$xtime = strtotime(urldecode($_REQUEST['start_time']));
			$map['l.dateline'] = array("gt",$xtime);
			$search['start_time'] = $xtime;
		}elseif(!empty($_REQUEST['end_time'])){
			$xtime = strtotime(urldecode($_REQUEST['end_time']));
			$map['l.dateline'] = array("lt",$xtime);
			$search['end_time'] = $xtime;
		}

		//分页处理
		import("ORG.Util.Page");
		$count = M('yott_money_log l')->join('lzh_members m on l.uid=m.id')->join('lzh_member_yott my on my.uid=l.uid')->count();
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		$list = M('yott_money_log l')->field('l.*,m.user_name,my.status as res,lz.borrow_name')->join('lzh_borrow_info lz on l.bid=lz.id')->join('lzh_members m on l.uid=m.id')->join('lzh_member_yott my on my.uid=l.uid')->where($map)->limit($Lsql)->order("l.datetime DESC")->select();

		$this->assign("status", array("1"=>'用户1',"2"=>'用户2'));
		$this->assign("res", array("1"=>'成功',"2"=>'失败'));
		$this->assign("list", $list);
		$this->assign("pagebar", $page);
		$this->assign("search", $search);
		$this->assign("query", http_build_query($search));
	...
	public function exportyott(){
		...
		$list = M('yott_money_log l')->field('l.*,m.user_name,my.status as res,lz.borrow_name')->join('lzh_borrow_info lz on l.bid=lz.id')->join('lzh_members m on l.uid=m.id')->join('lzh_member_yott my on my.uid=l.uid')->where($map)->order("l.datetime DESC")->select();


		$row=array();
		$row[0]=array('ID','用户名','会员类型','投资金额','投资时间','状态');
		$i=1;
		foreach($list as $v){
			$row[$i]['0'] = $v['id'];
			$row[$i]['1'] = $v['user_name'];
			$row[$i]['2'] = '用户'.$v['res'];
			$row[$i]['3'] = $v['account'];
			$row[$i]['4'] = date('Y-m-d H:i',$v['datetime']);
			$row[$i]['5'] = $v['status'] ? '成功' : '失败';
			$i++;
		}
		$xls = new Excel_XML('UTF-8', false, 'datalist');
		$xls->addArray($row);
		$xls->generateXML("yott_".date('Y-m-d'));
	}
	public function export(){
		...
		*/
		...
		foreach($list as $key=>$v){
			...
			$temp_arr  =  get_personal_benefit($uid);
			$money['jzlx'] = $temp_arr['interest'];
			$money['jflx'] = $borrowANDpaid[0]['interest'];//净付利息
			$money['ljjj'] = 0.00;//$umoney['reward_money'];//累计收到奖金
