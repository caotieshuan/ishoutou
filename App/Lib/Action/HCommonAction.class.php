<?php
// 全局设置
class HCommonAction extends GCommonAction
{
	var $glo=NULL;
	//上传参数
	...
	function _Uploada(){
		...
		$upload->thumbMaxHeight = $this->thumbMaxHeight;
		//var_dump($upload);die;
		$upload->maxSize  = 8000000 ;// 设置附件上传大小
		$upload->allowExts  =array('jpg', 'gif', 'png', 'jpeg');// 设置附件上传类型
		
		$upload->savePath =  $this->savePathNew?$this->savePathNew:'UF/Uploads/';// 设置附件上传目录
		
		if(!$upload->upload()) {// 上传错误提示错误信息
		    dump($upload->getErrorMsg());die;
			$this->error($upload->getErrorMsg());
		}else{// 上传成功 获取上传文件信息
		    echo "success";die;
			$info =  $upload->getUploadFileInfo();
