...
function getBorrowList($map,$size,$limit=10){
	...
	foreach($list as $key=>$v){
		...
		if($map['borrow_status']==6){
			...
			$list[$key]['cur'] = strtotime(date('Y-m-d',$vx['deadline'])) == strtotime(date('Y-m-d'));
		}
...
function getTenderList($map,$size,$limit=10){
	...
	$list=$Model->field(true)->where($map)->order('invest_time desc')->group('id')->limit($Lsql)->select();
	////////////////////////视图查询 fan 20130522//////////////////////////////////////////
	foreach($list as $key=>$v){
		//if($map['i.status']==4){
		if($map['status']==4){
			$list[$key]['total'] = ($v['borrow_type']==3 || in_array($v['repayment_type'],array(1,5)))?"1":$v['borrow_duration'];
