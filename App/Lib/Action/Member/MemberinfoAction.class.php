...
class MemberinfoAction extends MCommonAction {
	...
	public function base(){
		if(ListMobile()){
			$this->assign('edu',explode(',','高中以下,大专或本科,硕士或硕士以上'));
			$this->assign('income',explode(',','5000以下,5000-10000,10000-50000,50000以上'));
			$vo = $this->editmemberinfo(true);
			$prov ='请选择...';
			if($vo['province']){
				$prov = '';
				foreach(M('area')->field('name')->where(array('id'=>array('in',array($vo['province'],$vo['city'],$vo['area']))))->select() as $v){
					$prov .= $v['name']." ";
				}
			}
			$prov_now ='请选择...';
			if($vo['province_now']){
				$prov_now = '';
				foreach(M('area')->field('name')->where(array('id'=>array('in',array($vo['province_now'],$vo['city_now'],$vo['area_now']))))->select() as $v){
					$prov_now .= $v['name']." ";
				}
			}

			$this->assign('ivo',array('prov'=>$prov,'prov_now'=>$prov_now));
			$this->display();
		}
	}
	
	public function editmemberinfo($re = false){
		...
		if(!$_POST){
			...
			if(true === $re){
				$this->assign('mm',M('members')->find($this->uid));
				$this->assign('bb',M('member_banks')->where('uid='.$this->uid)->getfield('bank_num'));
				return $vo;
			}
		...
		unset($savedata['real_name'],$savedata['idcard'],$savedata['card_img'],$savedata['card_credits'],$savedata['cell_phone']);
