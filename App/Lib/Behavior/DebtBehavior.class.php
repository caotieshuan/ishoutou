<?php
    ...
    * @copyright rongtianxia 旗舰版
    * @link www.rongtianxia.com
    */
   class DebtBehavior
   {
       public $uid;
       ...
       */
        ...
        */
        public function listAll($parm,$size=9, $pstatus=true)
        {
            ...
            $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,d.addtime,
		 ...
		 public function listAllz($parm,$size=9, $pstatus=true)
        {
            if(empty($parm['map'])) return;
            $map= $parm['map'];
            $map['d.status']=array("in","2,4");
			...
			$suffix=C("URL_HTML_SUFFIX");
			 $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,d.addtime,
                     i.investor_uid, i.deadline, b.id, b.borrow_name, b.borrow_interest_rate,b.borrow_status,b.borrow_duration,m.credits, m.user_name";
			$list=M("invest_detb d")->field($field)->join("{$this->pre}borrow_investor i ON d.invest_id=i.id")->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")->join("{$this->pre}members m ON i.investor_uid=m.id")->where($map)->order($orderby)->limit($limit)->select();
			return $list;
        ...
        */
		...
		//wap端债权转让
		 public function buya($paypass, $invest_id)
        {
            $check_result = $this->checkBuya($paypass, $invest_id);
            ...
            if($check_result === 'TRUE'){
                ...
                //$investor_status = M("borrow_investor")->where("id={$invest_id}")->save(array('debt_uid'=>$this->uid));
               //$detail_status = M("investor_detail")->where("invest_id={$invest_id} and status in (6,7,0)")->save(array('investor_uid'=>$this->uid));
                $debt_status = $this->Model->where("invest_id={$invest_id}")->save($debt);
                $sell_user_status = M("member_money")->where("uid={$debt_info['sell_uid']}")->save($sell_user);
                $buy_user_status = M("member_money")->where("uid={$this->uid}")->save($buy_user);
				
				
                if($debt_status && $sell_user_status && $buy_user_status){
                    $this->Model->commit();
                    $this->moneyLog($this->uid, 46, -$debt_info['transfer_price'], $debt_info['money'], "购买{$serialid}号债权", $debt_info['sell_uid']);
                    $this->moneyLog($debt_info['sell_uid'], 47, $debt_info['transfer_price'], $debt_info['money'], "转让{$serialid}号债权", $this->uid);
                    memberMoneyLog($debt_info['sell_uid'],48,-$fee,"转让{$serialid}号债权手续费（转让金额的{$debt_fee}%）");
        ...
        */
		...
		private function checkBuya($paypass, $invest_id)
        {
            
            $invest = $this->Model->field("transfer_price, sell_uid, valid")->where("invest_id = {$invest_id}")->find();
            if($invest['sell_uid']==$this->uid){
                //return 'creditors_can_not_buy';
				return '不能购买自己发布的债权';exit;
            }
            if(time() >= $invest['valid']){
                //return 'debt_timeout';
				return '本债权转让已过期';;exit;
            }
            $isBorrow = M("borrow_investor")->where("id = {$invest_id} and borrow_uid = {$this->uid} ")->count();
            if($isBorrow){
                //return 'debtor_can_not_buy';
				return '原借款人不能购买债权';;exit;
            }
